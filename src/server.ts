...
(async () => {
  ...
  app.get("/filteredimage/", async (req, res) => {
    ...
    if (!Util.isValidImageUrl(image_url)) {
      return res.status(400)
      .send(`Invalid image url provided. Allowed protocols: http, https; Allowed image formats: .jpg,.jpeg,.gif,.png`);
    }
    ...
    filteredUrlPath = await Util.filterImageFromURL(image_url).catch((error) => console.error('Image processing failed: ', error));
